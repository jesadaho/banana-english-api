/**
 * About Me PoolGate — all 10 lessons, scenarios 1–5 against prod.
 *
 * Transcripts are always written under:
 *   about-me-scenario-results/YYYY-MM-DD/
 ...
 *     npx tsx scripts/about-me-scenarios-prod.ts
 *
 *   npx tsx scripts/about-me-scenarios-prod.ts ee_about_me_food 1
 *   npx tsx scripts/about-me-scenarios-prod.ts ee_about_me_food ee_about_me_home
 *   npx tsx scripts/about-me-scenarios-prod.ts ee_about_me_food ee_about_me_home 3
 *   npx tsx scripts/about-me-scenarios-prod.ts 3              # all lessons, scenario 3
 *   npx tsx scripts/about-me-scenarios-prod.ts ee_about_me_daily_routine
 */
...
import {
  formatMs,
  parseAboutMeScenarioArgs,
  runAboutMeScenario,
  SCENARIO_TITLES,
  type ScenarioRunResult,
} from './lib/about-me-scenarios-prod-runner';
...
try {
  ({ lessonIds, scenarios } = parseAboutMeScenarioArgs(process.argv));
} catch (err) {
  console.error(
    'Usage: npx tsx scripts/about-me-scenarios-prod.ts [lessonId ...] [scenario 1-5]',
  );
  console.error(
    '       npx tsx scripts/about-me-scenarios-prod.ts ee_about_me_food ee_about_me_home',
  );
  console.error(
    '       npx tsx scripts/about-me-scenarios-prod.ts [scenario 1-5]   # all lessons',
  );
...
function openResultLog(now = new Date()) {
  ...
  const dir = join(process.cwd(), 'about-me-scenario-results', date);
  ...
  try {
    for (const lessonId of lessonIds) {
      for (const scenario of scenarios) {
        results.push(
          await runAboutMeScenario(API_BASE, lessonId, scenario, runIndex++),
        );
      }
