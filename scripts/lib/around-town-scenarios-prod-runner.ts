...
import {
  aroundTownOutOfPoolCloseMiss,
  aroundTownOutOfPoolWrong,
...
} from '../../src/training/scripts/around-town.registry.ts';
...
export const ALL_AROUND_TOWN_SCENARIO_LESSON_IDS = AROUND_TOWN_CHOICE_LESSONS.map(
  (d) => d.lessonId,
) as readonly AroundTownScenarioLessonId[];
...
function pickUserSpeech(
  ...
  switch (scenario) {
    ...
    case 3:
      return { speech: aroundTownOutOfPoolCloseMiss(expected, step, lessonId) };
    case 4:
      return {
        speech: aroundTownOutOfPoolWrong(expected, step),
        recoverExact: expected,
      };
    case 5:
      return {
        speech: aroundTownOutOfPoolWrong(expected, step),
        recoverWrong: aroundTownOutOfPoolWrongAgain(expected, step),
      };
    default:
      return { speech: expected };
  }
...
export async function runAroundTownScenario(
  apiBase: string,
  lessonId: AroundTownScenarioLessonId,
  scenario: number,
  runIndex: number,
): Promise<ScenarioRunResult> {
  const def = getAroundTownChoiceLesson(lessonId);
  if (!def) {
    return {
      lessonId,
      scenario,
      ...
      steps: 0,
      totalMs: 0,
      error: `unknown Around Town lesson: ${lessonId}`,
    };
  }
...
export function parseAroundTownScenarioArgs(argv: string[]): {
  lessonIds: AroundTownScenarioLessonId[];
  scenarios: number[];
} {
  const args = argv.slice(2);
  const known = ALL_AROUND_TOWN_SCENARIO_LESSON_IDS as readonly string[];
  const lessonIds: AroundTownScenarioLessonId[] = [];
  ...
  for (const arg of args) {
    ...
    if (!lessonIds.includes(arg as AroundTownScenarioLessonId)) {
      lessonIds.push(arg as AroundTownScenarioLessonId);
    }
  }
  ...
  return {
    lessonIds:
      lessonIds.length > 0
        ? lessonIds
        : [...ALL_AROUND_TOWN_SCENARIO_LESSON_IDS],
