...
import {
  aboutMeOutOfPoolCloseMiss,
  aboutMeOutOfPoolWrong,
  aboutMeOutOfPoolWrongAgain,
  introductionsOutOfPoolNearMiss,
} from '../../src/training/about-me/about-me-poolgate.harness.ts';
import {
  ABOUT_ME_CHOICE_LESSONS,
  getAboutMeChoiceLesson,
} from '../../src/training/scripts/about-me.registry.ts';
...
export const ALL_ABOUT_ME_SCENARIO_LESSON_IDS = ABOUT_ME_CHOICE_LESSONS.map(
  (d) => d.lessonId,
) as readonly AboutMeScenarioLessonId[];

export type AboutMeScenarioLessonId =
  (typeof ALL_ABOUT_ME_SCENARIO_LESSON_IDS)[number];

/** Favorites teaching ends before roleplay — same as happy-path smoke. */
export const ABOUT_ME_TEACHING_ONLY_LESSONS = new Set<string>([
...
export function classifyResult(json: Json): string {
  ...
  if (dbg?.source === 'scripted') {
    if (tier === 'incorrect' && /ตรงนี้(พูด(ว่า|ได้ว่า)|ใช้โครง)/.test(String(json.aiResponse ?? ''))) {
...
function pickUserSpeech(
  ...
  switch (scenario) {
    ...
    case 3:
      return { speech: aboutMeOutOfPoolCloseMiss(expected, step, lessonId) };
    case 4:
      return {
        speech: aboutMeOutOfPoolWrong(expected, step),
        recoverExact: expected,
      };
    case 5:
      return {
        speech: aboutMeOutOfPoolWrong(expected, step),
        recoverWrong: aboutMeOutOfPoolWrongAgain(expected, step),
      };
    default:
      return { speech: expected };
  }
...
function isTeachingComplete(
  ...
  if (!ABOUT_ME_TEACHING_ONLY_LESSONS.has(lessonId)) return false;
...
export async function runAboutMeScenario(
  apiBase: string,
  lessonId: AboutMeScenarioLessonId,
  scenario: number,
  runIndex: number,
): Promise<ScenarioRunResult> {
  const def = getAboutMeChoiceLesson(lessonId);
  if (!def) {
    return {
      lessonId,
      scenario,
      ...
      steps: 0,
      totalMs: 0,
      error: `unknown About Me lesson: ${lessonId}`,
    };
  }
...
export function parseAboutMeScenarioArgs(argv: string[]): {
  lessonIds: AboutMeScenarioLessonId[];
  scenarios: number[];
} {
  const args = argv.slice(2);
  const known = ALL_ABOUT_ME_SCENARIO_LESSON_IDS as readonly string[];
  const lessonIds: AboutMeScenarioLessonId[] = [];
  ...
  for (const arg of args) {
    ...
    if (!lessonIds.includes(arg as AboutMeScenarioLessonId)) {
      lessonIds.push(arg as AboutMeScenarioLessonId);
    }
  }
  ...
  return {
    lessonIds:
      lessonIds.length > 0
        ? lessonIds
        : [...ALL_ABOUT_ME_SCENARIO_LESSON_IDS],
