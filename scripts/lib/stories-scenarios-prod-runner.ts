...
import {
  storiesOutOfPoolCloseMiss,
  storiesOutOfPoolWrong,
  storiesOutOfPoolWrongAgain,
  introductionsOutOfPoolNearMiss,
} from '../../src/training/stories/stories-poolgate.harness.ts';
import {
  STORIES_CHOICE_LESSONS,
  getStoriesChoiceLesson,
} from '../../src/training/scripts/stories.registry.ts';
...
export const ALL_STORIES_SCENARIO_LESSON_IDS = STORIES_CHOICE_LESSONS.map(
  (d) => d.lessonId,
) as readonly StoriesScenarioLessonId[];
...
function pickUserSpeech(
  ...
  switch (scenario) {
    ...
    case 3:
      return { speech: storiesOutOfPoolCloseMiss(expected, step, lessonId) };
    case 4:
      return {
        speech: storiesOutOfPoolWrong(expected, step),
        recoverExact: expected,
      };
    case 5:
      return {
        speech: storiesOutOfPoolWrong(expected, step),
        recoverWrong: storiesOutOfPoolWrongAgain(expected, step),
      };
    default:
      return { speech: expected };
  }
...
export async function runStoriesScenario(
  apiBase: string,
  lessonId: StoriesScenarioLessonId,
  scenario: number,
  runIndex: number,
): Promise<ScenarioRunResult> {
  const def = getStoriesChoiceLesson(lessonId);
  if (!def) {
    return {
      lessonId,
      scenario,
      ...
      steps: 0,
      totalMs: 0,
      error: `unknown Stories lesson: ${lessonId}`,
    };
  }
...
export function parseStoriesScenarioArgs(argv: string[]): {
  lessonIds: StoriesScenarioLessonId[];
  scenarios: number[];
} {
  const args = argv.slice(2);
  const known = ALL_STORIES_SCENARIO_LESSON_IDS as readonly string[];
  const lessonIds: StoriesScenarioLessonId[] = [];
  ...
  for (const arg of args) {
    ...
    if (!lessonIds.includes(arg as StoriesScenarioLessonId)) {
      lessonIds.push(arg as StoriesScenarioLessonId);
    }
  }
  ...
  return {
    lessonIds:
      lessonIds.length > 0
        ? lessonIds
        : [...ALL_STORIES_SCENARIO_LESSON_IDS],
