/**
 * Stories PoolGate — all 10 lessons, scenarios 1–5 against prod.
 *
 * Transcripts are always written under:
 *   stories-scenario-results/YYYY-MM-DD/
 ...
 *     npx tsx scripts/stories-scenarios-prod.ts
 *
 *   npx tsx scripts/stories-scenarios-prod.ts ee_stories_yesterday 1
 *   npx tsx scripts/stories-scenarios-prod.ts ee_stories_yesterday ee_stories_last_weekend
 *   npx tsx scripts/stories-scenarios-prod.ts ee_stories_yesterday ee_stories_last_weekend 3
 *   npx tsx scripts/stories-scenarios-prod.ts 3              # all lessons, scenario 3
 *   npx tsx scripts/stories-scenarios-prod.ts ee_stories_last_night
 */
...
import {
  formatMs,
  parseStoriesScenarioArgs,
  runStoriesScenario,
  SCENARIO_TITLES,
  type ScenarioRunResult,
} from './lib/stories-scenarios-prod-runner';
...
try {
  ({ lessonIds, scenarios } = parseStoriesScenarioArgs(process.argv));
} catch (err) {
  console.error(
    'Usage: npx tsx scripts/stories-scenarios-prod.ts [lessonId ...] [scenario 1-5]',
  );
  console.error(
    '       npx tsx scripts/stories-scenarios-prod.ts ee_stories_yesterday ee_stories_last_weekend',
  );
  console.error(
    '       npx tsx scripts/stories-scenarios-prod.ts [scenario 1-5]   # all lessons',
  );
...
function openResultLog(now = new Date()) {
  ...
  const dir = join(process.cwd(), 'stories-scenario-results', date);
  ...
  try {
    for (const lessonId of lessonIds) {
      for (const scenario of scenarios) {
        results.push(
          await runStoriesScenario(API_BASE, lessonId, scenario, runIndex++),
        );
      }
