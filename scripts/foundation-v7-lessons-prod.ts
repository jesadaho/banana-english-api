/**
 * Foundation V7 lessons — happy path to lesson complete on staging.
 *
 * Transcripts:
 ...
 *   npx tsx scripts/foundation-v7-lessons-prod.ts fnd_v7_u03n01
 */
...
try {
  lessonIds = parseFoundationV7LessonArgs(process.argv);
} catch (err) {
  console.error(
    'Usage: npx tsx scripts/foundation-v7-lessons-prod.ts [lessonId ... | authored | frozen | all | ch2 | u03]',
  );
  console.error(
    '       npx tsx scripts/foundation-v7-lessons-prod.ts fnd_v7_u03n01',
  );
  console.error('       npx tsx scripts/foundation-v7-lessons-prod.ts authored');
...
async function main(): Promise<void> {
  ...
  try {
    for (const lessonId of lessonIds) {
      results.push(await runFoundationV7Lesson(API_BASE, lessonId, runIndex++));
