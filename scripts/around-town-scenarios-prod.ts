/**
 * Around Town PoolGate — all 10 lessons, scenarios 1–5 against prod.
 *
 * Transcripts are always written under:
 *   around-town-scenario-results/YYYY-MM-DD/
 ...
 *     npx tsx scripts/around-town-scenarios-prod.ts
 *
 *   npx tsx scripts/around-town-scenarios-prod.ts ee_around_town_shopping 1
 *   npx tsx scripts/around-town-scenarios-prod.ts ee_around_town_shopping ee_around_town_restaurant
 *   npx tsx scripts/around-town-scenarios-prod.ts ee_around_town_shopping ee_around_town_restaurant 3
 *   npx tsx scripts/around-town-scenarios-prod.ts 3              # all lessons, scenario 3
 *   npx tsx scripts/around-town-scenarios-prod.ts ee_around_town_transport
 */
...
import {
  formatMs,
  parseAroundTownScenarioArgs,
  runAroundTownScenario,
  SCENARIO_TITLES,
  type ScenarioRunResult,
} from './lib/around-town-scenarios-prod-runner';
...
try {
  ({ lessonIds, scenarios } = parseAroundTownScenarioArgs(process.argv));
} catch (err) {
  console.error(
    'Usage: npx tsx scripts/around-town-scenarios-prod.ts [lessonId ...] [scenario 1-5]',
  );
  console.error(
    '       npx tsx scripts/around-town-scenarios-prod.ts ee_around_town_shopping ee_around_town_restaurant',
  );
  console.error(
    '       npx tsx scripts/around-town-scenarios-prod.ts [scenario 1-5]   # all lessons',
  );
...
function openResultLog(now = new Date()) {
  ...
  const dir = join(process.cwd(), 'around-town-scenario-results', date);
  ...
  try {
    for (const lessonId of lessonIds) {
      for (const scenario of scenarios) {
        results.push(
          await runAroundTownScenario(API_BASE, lessonId, scenario, runIndex++),
        );
      }
