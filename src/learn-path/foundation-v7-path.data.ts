...
export type FoundationV7NodeType = 'lesson' | 'say_it' | 'emoji_speak' | 'pronunciation' | 'describe_it' | 'story_bites' | 'conversation';
...
export function foundationV7NodeTypeCounts(): Record<FoundationV7NodeType, number> {
  const result = { lesson: 0, say_it: 0, emoji_speak: 0, pronunciation: 0, describe_it: 0, story_bites: 0, conversation: 0 };
...
for (const node of FOUNDATION_V7_NODES) {
  ...
  const canonical = node.type === 'say_it' && ref.topicId ? `say_it:${ref.topicId}`
    : node.type === 'emoji_speak' && ref.poolId ? `emoji_speak:${ref.poolId}` : null;
  if (canonical) {
